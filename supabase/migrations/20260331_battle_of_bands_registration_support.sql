  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'limit', case
          when normalized_event in ('codm-tournament', 'mobile-legends-tournament', 'battle-of-the-bands')
...
begin
  ...
  if normalized_event in ('codm-tournament', 'mobile-legends-tournament', 'battle-of-the-bands') then
  ...
  if normalized_event in ('rubiks-cube-competition', 'sudoku-game-easy-level', 'fast-typing', 'crimping-competition', 'assembling-and-disassembling-competition') then
    ...
    team_size_value := case
      ...
      when normalized_event = 'battle-of-the-bands' then array_length(normalized_members, 1) + 1
      else 4
    end;
