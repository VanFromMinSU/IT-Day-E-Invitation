...
begin
  for constraint_name in
    ...
    where conrelid = 'public.event_registrations'::regclass
      and contype = 'c'
      and (
        pg_get_constraintdef(oid) ilike '%event_id in (''rubiks-cube-competition'', ''sudoku-game-easy-level'', ''codm-tournament'')%'
        or (
          pg_get_constraintdef(oid) ilike '%registration_type = ''team''%'
          and pg_get_constraintdef(oid) ilike '%array_length(members, 1) = 3%'
          and pg_get_constraintdef(oid) ilike '%team_size = 4%'
        )
      )
  loop
    execute format('alter table public.event_registrations drop constraint %I', constraint_name);
  end loop;
end;
$$;
...
alter table public.event_registrations
  ...
  check (event_id in ('rubiks-cube-competition', 'sudoku-game-easy-level', 'codm-tournament', 'fast-typing'));
...
create policy registrations_insert_public
  ...
  with check (
    event_id in ('rubiks-cube-competition', 'sudoku-game-easy-level', 'codm-tournament', 'fast-typing')
  );
...
begin
  if normalized_event not in ('rubiks-cube-competition', 'sudoku-game-easy-level', 'codm-tournament', 'fast-typing') then
...
begin
  ...
  if total_count >= 8 then
    return jsonb_build_object(
      ...
      'message', case
        when normalized_event in ('codm-tournament', 'fast-typing') then 'Registration is now closed. Maximum teams reached.'
  ...
  if normalized_event in ('rubiks-cube-competition', 'sudoku-game-easy-level') then
    ...
    team_size_value := case
      when normalized_event = 'fast-typing' then 1 + coalesce(array_length(normalized_members, 1), 0)
      else 4
    end;
