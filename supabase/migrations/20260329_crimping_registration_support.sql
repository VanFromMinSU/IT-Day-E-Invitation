...
alter table public.event_registrations
  ...
  check (event_id in ('rubiks-cube-competition', 'sudoku-game-easy-level', 'codm-tournament', 'fast-typing', 'crimping-competition'));
  ...
  check (
    ...
    (
      ...
      and (
        ...
        or
        (
          event_id = 'fast-typing'
          and coalesce(array_length(members, 1), 0) between 0 and 2
          and team_size between 1 and 3
        )
        or
        (
          event_id = 'crimping-competition'
          and coalesce(array_length(members, 1), 0) between 0 and 1
          and team_size between 1 and 2
        )
      )
    )
  );
...
create policy registrations_insert_public
  ...
  with check (
    event_id in ('rubiks-cube-competition', 'sudoku-game-easy-level', 'codm-tournament', 'fast-typing', 'crimping-competition')
  );
...
begin
  if normalized_event not in ('rubiks-cube-competition', 'sudoku-game-easy-level', 'codm-tournament', 'fast-typing', 'crimping-competition') then
...
begin
  ...
  if total_count >= 8 then
    return jsonb_build_object(
      ...
      'message', case
        when normalized_event in ('codm-tournament', 'fast-typing', 'crimping-competition') then 'Registration is now closed. Maximum teams reached.'
  ...
  if normalized_event in ('rubiks-cube-competition', 'sudoku-game-easy-level') then
    ...
    if normalized_event = 'fast-typing' and coalesce(array_length(normalized_members, 1), 0) > 2 then
      return jsonb_build_object(
        'status', 400,
        'error', 'invalid_team_size',
        'message', 'Each team can register up to 3 participants including the Team Captain.',
        'state', public.get_registration_state(normalized_event, p_owner_token) - 'status'
      );
    end if;

    if normalized_event = 'crimping-competition' and coalesce(array_length(normalized_members, 1), 0) > 1 then
      return jsonb_build_object(
        'status', 400,
        'error', 'invalid_team_size',
        'message', 'Each team can register up to 2 participants including the Team Captain.',
        'state', public.get_registration_state(normalized_event, p_owner_token) - 'status'
      );
    end if;
    ...
    team_size_value := case
      when normalized_event in ('fast-typing', 'crimping-competition') then 1 + coalesce(array_length(normalized_members, 1), 0)
      else 4
    end;
