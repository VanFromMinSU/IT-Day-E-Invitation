...
alter table public.event_registrations
  ...
  check (
    ...
    (
      ...
      and (
        (
          event_id = 'chess-tournament'
          and array_length(members, 1) = 4
          and team_size = 4
        )
        or
        (
          event_id = 'it-quiz-bee'
          and array_length(members, 1) = 4
          and team_size = 5
        )
        or
        ...
        or
        (
          event_id = 'mr-and-ms-it-2026'
          and array_length(members, 1) = 1
          and team_size = 2
        )
      )
    )
...
as $$
  select case p_event_id
    ...
    when 'mr-and-ms-it-2026' then 'Mr. and Ms. IT 2026'
    else p_event_id
  end;
$$;
...
begin
  ...
  if normalized_event = 'chess-tournament' then
    team_family_limit := 1;
    team_max_teams := 4;
  elsif normalized_event = 'it-quiz-bee' then
    team_family_limit := 1;
    team_max_teams := 4;
  elsif normalized_event = 'battle-of-the-bands' then
    team_family_limit := 1;
    team_max_teams := 4;
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'limit', case
          when normalized_event in ('chess-tournament', 'it-quiz-bee', 'codm-tournament', 'mobile-legends-tournament', 'battle-of-the-bands', 'basketball-half-court', 'mr-and-ms-it-2026')
            then team_family_limit
          else individual_family_limit
        end,
        'remaining', case
          when normalized_event in ('chess-tournament', 'it-quiz-bee', 'codm-tournament', 'mobile-legends-tournament', 'battle-of-the-bands', 'basketball-half-court', 'mr-and-ms-it-2026')
            then greatest(0, team_family_limit - coalesce(counts.count, 0))
...
begin
  ...
  if normalized_event in ('chess-tournament', 'it-quiz-bee', 'codm-tournament', 'mobile-legends-tournament', 'battle-of-the-bands', 'basketball-half-court', 'mr-and-ms-it-2026') then
    if total_count >= team_max_teams then
      return jsonb_build_object(
        'status', 409,
  ...
  if normalized_event in ('rubiks-cube-competition', 'sudoku-game-easy-level', 'fast-typing', 'crimping-competition', 'assembling-and-disassembling-competition') then
    ...
    if family_count >= team_family_limit then
      return jsonb_build_object(
        'status', 409,
        'error', 'family_team_limit_reached',
        'message', case
          when normalized_event = 'battle-of-the-bands' then 'Only one (1) band registration is allowed per family.'
          when normalized_event = 'chess-tournament' then 'Only one (1) team registration is allowed per family.'
          when normalized_event = 'basketball-half-court' then 'Only one (1) team registration is allowed per family.'
          when normalized_event = 'it-quiz-bee' then 'Only one (1) team registration is allowed per family.'
          when normalized_event = 'mr-and-ms-it-2026' then 'Only one (1) Mr. and Ms. IT pair registration is allowed per family.'
          else 'This family has already registered the maximum number of teams.'
        end,
        'state', public.get_registration_state(normalized_event, p_owner_token) - 'status'
      );
    end if;
    ...
    if normalized_event = 'chess-tournament' and normalized_captain = '' then
      normalized_captain := 'Team Representative';
    end if;
    ...
    if normalized_captain = '' then
      return jsonb_build_object(
        ...
        'error', 'invalid_payload',
        'message', case
          when normalized_event = 'it-quiz-bee' then 'Please enter the watcher name.'
          when normalized_event = 'mr-and-ms-it-2026' then 'Please enter the Mr. IT candidate name.'
          else 'Please enter the captain/leader name.'
        end,
        'state', public.get_registration_state(normalized_event, p_owner_token) - 'status'
      );
    end if;
    ...
    if normalized_event = 'it-quiz-bee' and array_length(normalized_members, 1) is distinct from 4 then
      return jsonb_build_object(
        'status', 400,
        'error', 'invalid_team_size',
        'message', 'Each team must include one (1) watcher and four (4) participants.',
        'state', public.get_registration_state(normalized_event, p_owner_token) - 'status'
      );
    end if;

    if normalized_event = 'chess-tournament' and array_length(normalized_members, 1) is distinct from 4 then
      return jsonb_build_object(
        'status', 400,
        'error', 'invalid_team_size',
        'message', 'Each team must include exactly two (2) male and two (2) female participants.',
        'state', public.get_registration_state(normalized_event, p_owner_token) - 'status'
      );
    end if;
    ...
    if normalized_event = 'battle-of-the-bands' and (
      array_length(normalized_members, 1) is null
      or array_length(normalized_members, 1) < 4
      or array_length(normalized_members, 1) > 6
    ) then
      return jsonb_build_object(
        'status', 400,
        'error', 'invalid_team_size',
        'message', 'Each band must have 5 to 7 members, including the Band Leader.',
        'state', public.get_registration_state(normalized_event, p_owner_token) - 'status'
      );
    end if;

    if normalized_event = 'basketball-half-court' and (
      array_length(normalized_members, 1) is null
      or array_length(normalized_members, 1) < 2
      or array_length(normalized_members, 1) > 3
    ) then
      return jsonb_build_object(
        'status', 400,
        'error', 'invalid_team_size',
        'message', 'Each team must have 3 to 4 players, including the Captain/Leader.',
        'state', public.get_registration_state(normalized_event, p_owner_token) - 'status'
      );
    end if;

    if normalized_event = 'mr-and-ms-it-2026' and array_length(normalized_members, 1) is distinct from 1 then
      return jsonb_build_object(
        'status', 400,
        'error', 'invalid_team_size',
        'message', 'Each family must register exactly one (1) Mr. IT and one (1) Ms. IT candidate.',
        'state', public.get_registration_state(normalized_event, p_owner_token) - 'status'
      );
    end if;
    ...
    team_size_value := case
      when normalized_event = 'chess-tournament' then 4
      when normalized_event in ('it-quiz-bee', 'mobile-legends-tournament') then 5
      when normalized_event = 'mr-and-ms-it-2026' then 2
      when normalized_event in ('battle-of-the-bands', 'basketball-half-court') then array_length(normalized_members, 1) + 1
      else 4
    end;
