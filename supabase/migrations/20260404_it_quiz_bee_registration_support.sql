...
begin
  ...
  if normalized_event = 'it-quiz-bee' then
    team_family_limit := 1;
    team_max_teams := 4;
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'limit', case
          when normalized_event in ('it-quiz-bee', 'codm-tournament', 'mobile-legends-tournament', 'battle-of-the-bands', 'basketball-half-court')
...
begin
  ...
  if normalized_event in ('it-quiz-bee', 'codm-tournament', 'mobile-legends-tournament', 'battle-of-the-bands', 'basketball-half-court') then
