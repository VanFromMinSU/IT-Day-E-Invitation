  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'limit', case
          when normalized_event in ('chess-tournament', 'it-quiz-bee', 'codm-tournament', 'mobile-legends-tournament', 'battle-of-the-bands', 'basketball-half-court')
...
begin
  ...
  if normalized_event in ('chess-tournament', 'it-quiz-bee', 'codm-tournament', 'mobile-legends-tournament', 'battle-of-the-bands', 'basketball-half-court') then
  ...
  if normalized_event in ('rubiks-cube-competition', 'sudoku-game-easy-level', 'fast-typing', 'crimping-competition', 'assembling-and-disassembling-competition') then
    ...
    if normalized_captain = '' then
      return jsonb_build_object(
        'status', 400,
        'error', 'invalid_payload',
        'message', case when normalized_event = 'it-quiz-bee' then 'Please enter the watcher name.' else 'Please enter the captain/leader name.' end,
        'state', public.get_registration_state(normalized_event, p_owner_token) - 'status'
      );
    end if;
